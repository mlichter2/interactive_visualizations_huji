...
function drawEarthQuakes(geojson) {
    ...
    var dropDownBase = d3.select("#dropdown-base")
        .selectAll("a")
        .data(['Light', 'Dark'])
        .enter()
        .append("a")
        ...
        dropDownBase.on("click", function (d) {
            if (layer) {
            ...
            layer = basemapProviders['ESRI_World_' + d + '_Gray_Base'];
                map.addLayer(layer);
            updateData(minTime, maxTime, minMag, maxMag)
    ...
    var slider = createD3RangeSlider(minMag, maxMag, "#slider-container");
  
    slider.range(minMag, maxMag);

    slider.onChange(function (newRange) {

        minMag = newRange.begin
        maxMag = newRange.end
